      ...
      {Array.from({ length: 3 }).map((_, section) => (
        <div key={section} className="space-y-3">
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="flex items-center gap-4 rounded-lg border border-stone-100 p-4">
              <div className="h-5 w-5 shrink-0 animate-pulse rounded-full bg-stone-200" />
