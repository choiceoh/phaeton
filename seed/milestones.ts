...
// 루프탑 12단계
const rooftopTemplates: MilestoneTemplate[] = [
  { name: '사업타당성검토', category: 'admin', seqOrder: 1, typicalDays: 20, isCritical: false },
  {
    name: '건축물 구조검토',
  ...
  { name: '전기사업허가', category: 'admin', seqOrder: 3, typicalDays: 60, isCritical: true },
  { name: '설계 완료', category: 'engineering', seqOrder: 4, typicalDays: 30, isCritical: false },
  { name: '자재 발주', category: 'procurement', seqOrder: 5, typicalDays: 14, isCritical: false },
  ...
  { name: '모듈 설치', category: 'construction', seqOrder: 9, typicalDays: 14, isCritical: false },
  { name: '전기 공사', category: 'construction', seqOrder: 10, typicalDays: 14, isCritical: false },
  { name: '사용전검사', category: 'admin', seqOrder: 11, typicalDays: 14, isCritical: true },
...
export async function seedMilestoneTemplates(payload: Payload) {
  ...
  console.log(
    `[Phaeton] 마일스톤 템플릿 시드 완료: solar ${solarTemplates.length} + ` +
      `rooftop ${rooftopTemplates.length} + ess ${essTemplates.length} = ${total}건`,
  )
}
