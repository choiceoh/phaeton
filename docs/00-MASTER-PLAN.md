...
**목적:** 엑셀 기반 20+ 태양광·풍력·ESS 프로젝트 관리를 통합 웹 시스템으로 전환
**사용자:** 총 300명 (데이터 입력 100명, 열람 200명), 외부 인터넷 접속
...
- **직접 만드는 것:** 마일스톤 엔진, 포트폴리오 대시보드, 인력 배치 관리, 인허가 추적, Deneb 연동.
...
| staff | 인력 마스터 | staff 테이블 |
| staff-assignments | 프로젝트별 인력 배치 | staff_assignment 테이블 |
...
|-----------|---------------------|
| GiST EXCLUDE (인력 겹침 방지) | beforeChange 훅에서 TS로 중복 검증 + 커스텀 마이그레이션으로 DB 제약 추가 |
...
│   │   ├── validateAssignment.ts    # 인력 배치 과할당 검증
...
│   └── *_add_gist_exclude.ts        # GiST EXCLUDE 제약 추가
└── docs/
    ├── 00-MASTER-PLAN.md
    ├── 01-PAYLOAD-SETUP.md
    ├── 02-COLLECTIONS.md
...
- GiST EXCLUDE 제약은 커스텀 마이그레이션으로 추가. Hooks 검증과 이중 보호.
