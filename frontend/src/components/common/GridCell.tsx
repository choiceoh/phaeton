...
export default function GridCell({
  ...
  return (
    <div
      className={cn(
        'group/cell relative min-h-[28px] px-1 -mx-1 rounded-sm cursor-cell',
    ...
    >
      {children}
      {editable && !isActive && (
        <Pencil className="absolute right-1 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground opacity-0 group-hover/cell:opacity-100 transition-opacity" />
      )}
    </div>
  )
}
