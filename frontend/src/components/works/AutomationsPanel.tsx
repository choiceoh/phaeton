...
export default function AutomationsPanel({ collectionId }: AutomationsPanelProps) {
  const { data: collection } = useCollection(collectionId)
  const { data: automations, isLoading } = useAutomations(collectionId)
  const { data: process } = useProcess(collectionId)
  const createAutomation = useCreateAutomation(collectionId)
  const updateAutomation = useUpdateAutomation(collectionId)
  const deleteAutomation = useDeleteAutomation(collectionId)
    ...
    <div className="space-y-4">
      ...
      {formOpen && (
        <Card className="p-4 space-y-4">
          <h3 className="font-semibold text-sm">{editingId ? '자동화 수정' : '새 자동화'}</h3>
          ...
          <div>
            ...
            <Select value={triggerType} onValueChange={(v) => v && setTriggerType(v as TriggerType)}>
              ...
              <SelectContent>
                {(Object.keys(TRIGGER_LABELS) as (keyof typeof TRIGGER_LABELS)[]).map((t) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="outline" size="sm" onClick={addCondition}>+ 조건 추가</Button>
            </div>
            {conditions.length > 0 && (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="outline" size="sm" onClick={addAction}>+ 액션 추가</Button>
            </div>
            {actions.length > 0 && (
              <div className="space-y-3">
                {actions.map((a, idx) => (
      ...
      {(!automations || automations.length === 0) && !formOpen && (
        <div className="py-8 text-center text-muted-foreground">
          ...
          <p>아직 자동화가 없습니다</p>
          <p className="text-sm">데이터 변경 시 자동으로 알림을 보내거나 항목을 업데이트할 수 있습니다.</p>
        </div>
      )}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <Badge variant="secondary">{TRIGGER_LABELS[a.trigger_type as keyof typeof TRIGGER_LABELS] ?? a.trigger_type}</Badge>
