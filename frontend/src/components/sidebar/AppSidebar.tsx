...
export default function AppSidebar({ user }: { user: UserType }) {
  ...
  if (!isOpen) {
    return (
      <div className="flex h-full w-10 shrink-0 flex-col items-center border-r border-border/60 bg-white py-2">
        <button
          type="button"
  ...
  return (
    <div
      className="relative flex h-full shrink-0 flex-col border-r border-border/60 bg-white"
    ...
    >
      ...
      {/* Resize handle */}
      <div
        className={`absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/20 ${isResizing ? 'bg-primary/30' : ''}`}
